...
def load_commands(app: Flask) -> None:
    def get_db_config():
        ...
        INI_FILE = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'alembic.ini'))
        alembic_oonfig = AlembicConfig(INI_FILE)
        alembic_oonfig.set_main_option('sqlalchemy.url', app.config['SQLALCHEMY_DATABASE_URI'])
